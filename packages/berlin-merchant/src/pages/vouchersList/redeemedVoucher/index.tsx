...
const headCells: HeadCell[] = [
  ...
  {
    id: 'redemption_date_time',
    numeric: false,
    disablePadding: true,
    label: 'Voucher Redeemed Date',
  },
  {
    id: 'order_id',
    numeric: false,
...
export default function RedeemedVoucher() {
  ...
  const getRedeemedVoucherHandler = () => {
    setLoading(true);
  ...
  return (
    <>
      ...
      <Breadcrumbs aria-label="breadcrumb">
        ...
        <Typography color="text.primary">Redeemed Voucher List</Typography>{' '}
      </Breadcrumbs>
      <MainCard content={false} title="Redeemed Voucher List">
        <Box
          sx={{
            minWidth: 120,
            position: 'absolute',
  ...
  return (
    <>
      <TableRow hover tabIndex={-1} key={row?.id}>
        ...
        <TableCell>
          {row?.redemption_date_time
            ? DateFormat(row?.redemption_date_time)
            : null}
        </TableCell>
