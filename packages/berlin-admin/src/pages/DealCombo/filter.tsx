...
export function Filter({
  ...
  const setFilterHandler = () => {
    formik.setFieldValue('comboid', userFilterDetails?.filterDetails?.comboid);
    formik.setFieldValue('name', userFilterDetails?.filterDetails?.name);
    formik.setFieldValue(
      'combo_child_id',
      userFilterDetails?.filterDetails?.combo_child_id
    );
    formik.setFieldValue(
      'categoryid',
          ...
          <form onSubmit={formik.handleSubmit}>
            <Grid container width={200}>
              <Grid item mt={1} xs={12} sm={12}>
                <Stack spacing={1}>
                  <InputLabel>Deal Combo Id</InputLabel>
                  <NormalTextField
                    name="comboid"
                    id="outlined-basic"
                    variant="outlined"
                    placeholder="Enter Deal Combo Id"
                  />
                </Stack>
              </Grid>
              <Grid item mt={1} xs={12} sm={12}>
                <Stack spacing={1}>
                  <InputLabel>Deal Combo Name</InputLabel>
                  <NormalTextField
                    name="name"
                    id="outlined-basic"
                    variant="outlined"
                    placeholder="Enter Deal Combo Name"
                  />
                </Stack>
              </Grid>
              ...
              <Grid item xs={12} sm={12} mt={1}>
                <Stack spacing={1}>
                  <InputLabel>Combo Child Deals</InputLabel>
                  <MagicDropDown
                    name="combo_child_id"
                    option={dealList}
                    formik={formik}
                    placeholder="Select Combo Child Deals"
                  />
                </Stack>
              </Grid>
