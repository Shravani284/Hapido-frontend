...
import { getDealBundleByID } from '../../../services/dealBundleService';
...
function index() {
    ...
    getDealCalender(dealId)
      .then((response) => {
        ...
        const formattedData = result?.data.map((item) => {
          const originalDate = moment(item.date).format('YYYY-MM-DD');
          return {
            ...item,
            date: originalDate,
          };
  ...
  const assignInventorySlot = () => {
    ...
    if (
      isNaN(parseInt(assignedInventoryTotal.allocate)) ||
      isNaN(parseInt(assignedInventoryTotal.price))
    ) {
      setModalOpen({
        open: true,
    ...
    const payload = dealSlots?.map((e) => {
      const simplifiedDealTypePrices = e.dealTypePrices.map(
        ({ id, price }) => ({ id, price: parseInt(price) })
      );
