...
const URL = {
  UPLOADVIDEO: '/api/v1/uploader/videos?lang=en',
  UPLOADIMAGE: '/api/v1/uploader/images?lang=en',
...
export const deleteImgVideo = async (
  ...
  try {
    const response = await ApiRequest.delete(
      `${URL.DELETEIMGANDVIDEO}${imgOrVideoId}?for=${media}&table_name=${tName}&id=${tId}&lang=${lang}`
    );
    return response; // Assuming the API response contains the 'data' property
  } catch (error) {
    throw error; // Re-throw the error to handle it higher up in the call stack
