...
import CustomDropDownAr from '../../components/customDropDownWeb-ar';
...
const ScheduleAr = ({ productDetailsInfo }: any) => {
  ...
  const applyHandler = () => {
    ...
    const payload: any = getPayloadAr(
      selectedData,
      productDetailsInfo,
      deal_type
    );
  ...
  const packageHandler = (e) => {
    ...
    const component = getDealComponentAr(e, getCalendar);
    if (component === 'defaultCount') {
  ...
  const timeHandler = (e) => {
    ...
    const component = getTimeSelectionComponentAr(selectedData.package);
    if (component === 'defaultCount') {
      ...
      <Card className={`${css.card} ${css.MobCard} ${scrollHighlight}`}>
        <Box className={css.cardScheduleHeader} width={'100%'}>
          <Box>
            ...
            <span className={`${css.newRate} RTLnewRate`}>
              {t('NOW')}
