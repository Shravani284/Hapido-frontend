...
import ScheduleModule from '../../components/customDropdownMobile';
...
const MobileSchedule = ({ productDetailsInfo }) => {
  ...
  const timeHandler = (e) => {
    ...
    const component = getTimeSelectionComponent(productDetailsInfo);
    if (component === 'defaultCount') {
  ...
  const applyHandler = () => {
    ...
    const payload: any = getPayloadMobile(
      selectedData,
      productDetailsInfo,
      deal_type,
      id
  ...
  const getComponents = (value) => {
    switch (value) {
      case 'date':
        return (
          <>
            <ScheduleModule open={open} setOpen={setOpen} name="date">
              {loader ? (
                'Loading'
              ) : (
                <>
      ...
      case 'time':
        return (
          <ScheduleModule open={open} setOpen={setOpen} name="time">
            <>
              <h3>{t('SELECT_SLOT')}</h3>
              <ClockPopup
      ...
      case 'location':
        return (
          <ScheduleModule open={open} setOpen={setOpen} name="location">
            <>
