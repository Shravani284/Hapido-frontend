...
import ExclusiveDealsSlider from './components/exclusiveDeals';
import { useTranslation } from 'react-i18next';
import css from './HomePageStyle.module.scss';
import HotDeals from './components/hapidoHotDeals';
import TravelDeals from './components/travelDeals';
import MegaSales from './components/megaSales';
import StaycationDeals from './components/staycationDeals';
...
const Home = () => {
  ...
  const getComponent = (item: any) => {
    switch (item?.component) {
      ...
      case 'DOUBLE_COLUMN_CATEGORY':
        return <TravelDeals doubleColumn={item} />;
      case 'MEGASALE_BANNER':
        return (
          <>
            <Box className={css.megaSalesBanner}>
              {item?.images?.length > 0 && (
                <CarouselSlide
                  sliderHeading={item?.placement_title}
                  arrowPosition="top"
      ...
      case 'CIRCLE_BANNER':
        return <MegaSales images={item?.images} id={item?.target_deal_id} />;
      case 'SINGLE_COLUMN_CATEGORY':
        return <StaycationDeals singleColumn={item} />;
      default:
        break;
    }
  };
  ...
  return (
    <>
      ...
      {size < 768 ? (
        <ResponsiveHome homeLayoutPlacement={homeLayoutPlacement} />
      ) : (
        <>
          <Header />
            ...
            {isSkeleton ? (
              ...
              <Box className={css.sliderHome}>
                <CarouselSlide
                  ...
                  setting={{ autoplay: true, autoplaySpeed: 5000 }}
